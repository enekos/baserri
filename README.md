...
- **`arola`** runs on the Mac. It probes the box over ssh, tells you what is not
  yet true, and converges it one idempotent step at a time.
- **`arolad`** runs on the Pi. It long-polls Telegram, answers `/status`,
...
`curl` and the remote calls shell out to `ssh`/`scp`, exactly as `lantegi` did.
The result is one static ELF you `scp` and run.

```
cargo build --release --target aarch64-unknown-linux-musl --bin arolad
```

## Quickstart

```bash
arola init            # writes arola.conf + arolad.conf, both gitignored
$EDITOR arola.conf    # host, hostname, timezone, lan_cidr
$EDITOR arolad.conf   # bot token + your chat id (ask @userinfobot)

arola doctor          # what the box is, and what will bite
arola plan            # what apply would change, nothing written
arola apply           # converge
arola ship            # rebuild arolad and push just that
arola logs 100
```
...
failed check reports `blocked`, never `todo`, so arola cannot "fix" something it
could not measure.
...
2. **`arolad` does not run as root.** `/sh` runs as the `arola` system user. A
...
/df              filesystem usage
/jobs            the named jobs in arolad.conf
...
`arola.conf` (the Mac side) and `arolad.conf` (the box side) share one parser:
